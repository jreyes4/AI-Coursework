...
def axiom_generator_only_one_heading(heading='north', t=0):
    ...
    for idx in range(len(directions)):
        if heading != directions[idx]:
            not_heading.append('~{0}'.format(eval('state_heading_{0}_str({1})'.format(directions[idx], t))))
    axiom_str = eval('state_heading_{0}_str({1})'.format(heading, t))
    axiom_str += (' & {0}'.format(' & '.join(not_heading)))
    return axiom_str
...
def axiom_generator_only_one_action_axioms(t):
    ...
    for idx in range(len(actions)):
        initialAction = eval('action_{0}_str({1})'.format(actions[idx], t))
        noAction = []
